...
if [ -d "$SD1_ROM" ]; then
    ...
    if [ $CURRENT_VER = "PREBANANA" ]; then
        echo -e "Size of ROM Folder: $ROM_SIZE MB\n" >/tmp/muxlog_info
    fi
fi
if [ -d $SD1_BIOS ]; then
...
# Create necessary directories on SD2
echo "Creating necessary directories on SD2..."
mkdir -p "$SD2_ROM"
chmod 777 "$SD2_ROM"

# Transfer ROMs folder first as it's typically the largest
if [ -d "$SD1_ROM" ]; then
    echo -e "\nCopying ROMs to SD Card 2"
    if [ $CURRENT_VER = "PREBANANA" ]; then
        echo -e "Copying ROMs to SD Card 2\n" >/tmp/muxlog_info
    fi
    sleep 1
    ...
    # Use rsync with trailing slash to copy contents
    rsync $RSYNC_OPTS "${SD1_ROM}/" "${SD2_ROM}/"
    
    # Verify the transfer
    if [ $? -eq 0 ]; then
        echo "ROMs transfer completed successfully"
    else
        echo "Warning: ROMs transfer may have encountered issues"
    fi
else
    echo -e "\nNo ROMs folder exists on SD1, creating empty directory on SD2"
    mkdir -p "$SD2_ROM"
    chmod 777 "$SD2_ROM"
fi
